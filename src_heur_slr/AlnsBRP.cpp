...
void ALNS::Optimize(Sol & s, BestSolutionList * best_sol_list)
{
	...
	int best_drv_cnt = s.GetUsedDriverCount();
	...
	for(int iter = 0 ; iter < _iterator_count ;iter++)
	{
		...
		ins->opt->Insert(cur,false); //Does show = false;

		double newcost = cur.GetCost();
		
		double elapsed_time = (double)(clock() - begin_time) / CLOCKS_PER_SEC;
		
		if(elapsed_time >= _max_time)
		{
			printf("elapsed:%.1lf exceeded max_time:%d Exiting ...\n",elapsed_time,_max_time); break;
		}
		...
		if(best_cost > newcost && cur.IsFeasible() && _recourse_policy==2 && cur.GetUsedDriverCount() > best_drv_cnt)
		{
			cur.MergeAllPaths();//Checks sequentially for pairs of route that can be merged if they respect distance constraint
			newcost = cur.GetCost();
		}
		
		if(best_cost > newcost && cur.IsFeasible())
		{
			if(s.GetUsedDriverCount() > best_drv_cnt && _recourse_policy == 2)
			{
				printf("best_cost:%.2lf drvs:%d new_cost:%.2lf drvs:%d gap_best:%.2lf\n",best_cost,best.GetUsedDriverCount(),newcost,cur.GetUsedDriverCount(),gap_best);
				//best.Show();
				//cur.Show();
			}
			best_drv_cnt = s.GetUsedDriverCount();
